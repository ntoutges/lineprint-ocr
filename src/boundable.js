...
exports.getAverageCharBounds = exports.getTopLeftCharBounds = exports.getLineCharBoundsBlind = exports.getTopLine = exports.getLineFirstCharBounds = exports.getLineCharBounds = void 0;
...
function getLineCharBounds(img, avgCharBounds, firstCharBounds, lastCharBoundsList) {
    ...
    const vLookaroundD = (0, settings_js_1.getSetting)("charBounds.lookaround.vertical-down");
    const hLookaround = (0, settings_js_1.getSetting)("charBounds.lookaround.horizontal");
    const maxX = img.bitmap.width;
    let lastCharBounds = firstCharBounds;
    for (let i = 0; i < 200; i++) { // while(true), with built in limit
        ...
        let charBounds = getTopLeftCharBounds(img, minX, maxX, minY, maxY);
        if (!charBounds)
            break;
        ...
        if (widthFactor > 1) {
            ...
            charBounds = getClosestBoundsAfter(lastCharBounds, boundCandidates);
        }
        ...
        if (heightFactor > 1.3) {
            ...
            //   boundCandidates
            // );
            charBounds = splitBoundVerticallyByReference(charBounds, lastCharBounds);
        }
...
function rebound(img, bounds) {
    const blacklist = new Set();
    img.scan(bounds.x, bounds.y, bounds.w, bounds.h, (x, y, idx) => {
        if (img.bitmap.data[idx] == 0xFF || blacklist.has(idx / 4))
            return;
...
function getClosestBounds(b1, bn) {
    let distSq = Infinity;
    let closest = null;
    for (const b2 of bn) {
...
function getClosestBoundsAfter(b1, bn // must be sorted (furthest left is the first)
) {
    for (const b2 of bn) {
        if (b2.x >= b1.x2)
            return b2;
...
function doBoundsOverlap(b1, b2) {
    return (b1.x >= b2.x && b1.x <= b2.x + b2.w || b2.x >= b1.x && b2.x <= b1.x + b1.w)
        && (b1.y >= b2.y && b1.y <= b2.y + b2.h || b2.y >= b1.y && b2.y <= b1.y + b1.h);
}
function getOverlappingBound(b1, bn) {
    for (const b2 of bn) {
        if (doBoundsOverlap(b1, b2))
            return b2;
    }
    return null;
}
function getLineFirstCharBounds(img, avgCharBounds) {
    const lookaroundU = (0, settings_js_1.getSetting)("charBounds.lookaround.vertical-up");
    const lookaroundD = (0, settings_js_1.getSetting)("charBounds.lookaround.vertical-down");
    const yBuffer = (0, settings_js_1.getSetting)("charBounds.yBuffer.total");
    const yBufferOffset = (0, settings_js_1.getSetting)("charBounds.yBuffer.total-offset");
    const boundsList = [];
    let minY = yBuffer;
...
function splitBoundVertically(img, bounds, resultant, lookaroundU, lookaroundD) {
    if (resultant <= 1)
        return [bounds];
...
function splitBoundVerticallyByReference(bounds, refBounds) {
...
function splitBoundHorizontally(img, bounds, resultant, lookaround) {
    if (resultant <= 1)
        return [bounds];
...
function getLineCharBoundsBlind(img, minY, maxY) {
    const boundsList = [];
    let minX = 0;
    const maxX = img.bitmap.width;
...
exports.getLineCharBoundsBlind = getLineCharBoundsBlind;
...
function getAverageCharBounds(boundsList) {
    ...
    for (const bounds of boundsList) {
        ...
        lastBounds = bounds;
        if (spaceDistance > bounds.w)
            continue; // some amount of spaces in between; not just gap between characters
